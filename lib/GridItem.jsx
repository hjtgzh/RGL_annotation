...
export default class GridItem extends React.Component {
  ...
  getPositionParams(props = this.props) {
    return {
      // 布局中的列数
      cols: props.cols,
      // 容器填充（以像素为单位）。可以指定为水平和垂直填充，例如“[10，10]”，也可以指定为断点->containerPadding映射，例如“{lg:[10，0]，md:[10、10]，…}”。
      containerPadding: props.containerPadding,
      containerWidth: props.containerWidth,
      // 项目之间的边距[x，y]（以像素为单位）。
      margin: props.margin,
      maxRows: props.maxRows,
      // 网格单元的高度大小基于rowHeight，实际像素高度是(rowHeight * h) + (marginH * (h - 1)，marginH对应的margin
  ...
  onResizeHandler(
    e,
    { node, size, handle }, // 'size' is updated position
