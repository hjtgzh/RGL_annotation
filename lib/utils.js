...
export function compact(
  ...
  console.log('compact', { layout: JSON.stringify(layout), compactType })
...
function resolveCompactionCollision(
  ...
  for (let i = itemIndex + 1; i < layout.length; i++) {
    ...
    //我们可以这样做b/c这是一个排序布局
    console.log('itemIndex---resolveCompactionCollision', {
      i,
      item: JSON.stringify(item),
      otherItem: JSON.stringify(otherItem),
      collides: collides(item, otherItem)
    })
    ...
    if (collides(item, otherItem)) {
      console.log('otherItem---resolveCompactionCollision', { item: JSON.stringify(item), otherItem: JSON.stringify(otherItem) })
...
export function compactItem(
  ...
  while (
    ...
    console.log('compactItem', {
      fullLayout: JSON.stringify(fullLayout),
      compareWith: JSON.stringify(compareWith),
      collides_getFirstCollision: JSON.stringify(collides),
      l: JSON.stringify(l)
    })
    if (compactH) {
...
export function moveElement(
  ...
  console.log('moveElement', { layout: JSON.stringify(layout), collisions, sorted, movingUp })
...
export function moveElementAwayFromCollision(
  ...
  if (isUserAction) {
    ...
    console.log('moveElementAwayFromCollision', {
      collidesWith: JSON.stringify(collidesWith),
      itemToMove: JSON.stringify(itemToMove),
      firstCollision: JSON.stringify(firstCollision),
      fakeItem: JSON.stringify(fakeItem),
      compactH,
      compactV,
      firstCollision_collidesWith_Equal: deepEqual(firstCollision, collidesWith),
      firstCollision_itemToMove_Equal: deepEqual(firstCollision, itemToMove),
      layout: JSON.stringify(layout),
      getAllCollisions: getAllCollisions(layout, fakeItem)
    })
    // 第一个发生碰撞的布局项的底部边界（y + h）大于碰撞项的 y 坐标
    ...
    if (!firstCollision) {
      ...
      // 6、改变移动元素跟碰撞元素的位置
      // 这里把正在移动元素的 y 调整为碰撞元素的 y 的坐标
      collidesWith.y = itemToMove.y;

      // TODO:(janko) 解决拖入后下面边距过大的问题?
      // 这里将碰撞元素，向下移动自己 h 的距离
      itemToMove.y = itemToMove.y + itemToMove.h;

      console.log('collisionNorth && compactType == null', {
        layout: JSON.stringify(layout)
      })
      return layout;
